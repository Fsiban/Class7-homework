...
print(f"Coeficients per class: {len(lr.coef_)}\n")

print(f"Available classes: {lr.classes_}\n")
print(f"Named Coeficients for class 0: {pd.DataFrame(lr.coef_[-1], columns_names)}\n")
...
for (real, predicted) in list(zip(y_test, predicted_values)):
    print(f'Value: {real}, pred: {predicted} {"Oops missed!!" if real != predicted else ""}')
...
proj = pca.fit_transform(cancer.data)
plt.scatter(proj[:, 0], proj[:, 1], c=cancer.target, edgecolors='black')
plt.colorbar()
plt.savefig(f'plots/class7/cancer_dataset_in_2D.png', format='png')
